...
*Setup instructions for a Gitian build of BCS Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BCS
...
- [Building BCS Core](#building-bcs-core)
...
Clone the git repositories for bcs and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Blockchain-Solutions-BCS/bcs --recursive
git clone https://github.com/Blockchain-Solutions-BCS/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/bcs/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the bcs repository under 'Fetch and create inputs' to install sources which require
...
To build BCS Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bcs repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bcs/.git/
...
For example:
```bash
URL=https://github.com/laanwj/bcs.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bcs=${COMMIT} --url bcs=${URL} ../bcs/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bcs=${COMMIT} --url bcs=${URL} ../bcs/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bcs=${COMMIT} --url bcs=${URL} ../bcs/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-bcs:
./bin/gbuild --commit bcs=${COMMIT},cpp-eth-bcs=${ETHCOMMIT} --url bcs=${URL},cpp-eth-bcs=${ETHURL} ../bcs/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the bcs git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bcs/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/Blockchain-Solutions-BCS/bcs-detached-sigs.git

BTCPATH=/some/root/path/bcs
SIGPATH=/some/root/path/bcs-detached-sigs

./bin/gbuild --url bcs=${BTCPATH},signature=${SIGPATH} ../bcs/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bcs-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bcs-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bcs-osx-build.assert
```
...
[Blockchain-Solutions-BCS/gitian.sigs](https://github.com/Blockchain-Solutions-BCS/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@bcs.org) and he will commit them.
