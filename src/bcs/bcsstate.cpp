...
BCSState::BCSState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BCSState::openDB(_path + "/bcsDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BCSState::BCSState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BCSState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BCSTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBCSAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            bcs::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> BCSState::vins() const // temp
{
...
void BCSState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BCSState::vin(dev::Address const& _a) const
{
    return const_cast<BCSState*>(this)->vin(_a);
}

Vin* BCSState::vin(dev::Address const& _addr)
{
...
// void BCSState::commit(CommitBehaviour _commitBehaviour)
// {
...
void BCSState::kill(dev::Address _addr)
{
...
void BCSState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void BCSState::deleteAccounts(std::set<dev::Address>& addrs){
...
void BCSState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BCSState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void BCSState::validateTransfersWithChangeLog(){
