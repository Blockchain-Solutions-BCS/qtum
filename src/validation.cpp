...
std::unique_ptr<BCSState> globalState;
...
const std::string strMessageMagic = "BCS Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = bcsDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            ...
            BCSTxConverter converter(tx, NULL, NULL, contractflags);
            ExtractBCSTX resultConverter;
            if(!converter.extractionBCSTransactions(resultConverter)){
            ...
            std::vector<BCSTransaction> bcsTransactions = resultConverter.first;
            std::vector<EthTransactionParams> bcsETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(BCSTransaction bcsTransaction : bcsTransactions){
                sumGas += bcsTransaction.gas() * bcsTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, bcsTransaction.gasPrice());
                } else {
                    txMinGasPrice = bcsTransaction.gasPrice();
                }
                VersionVM v = bcsTransaction.getVersion();
                if(v.format!=0)
                ...
                if(bcsTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(bcsTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(bcsTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)bcsTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(bcsETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > bcsTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BCS additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // bcs
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // bcs
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, CBlockIndex* pblockindex, const dev::Address& sender, uint64_t gasLimit) {
    ...
    BCSTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<BCSTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path bcsDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(bcsDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(bcsDir.string(), std::ios::in | std::ios::out);
...
bool BCSTxConverter::extractionBCSTransactions(ExtractBCSTX& bcstx){
    ...
    std::vector<BCSTransaction> resultTX;
    ...
    bcstx = std::make_pair(resultTX, resultETP);
    return true;
}

bool BCSTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool BCSTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
BCSTransaction BCSTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    BCSTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = BCSTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = BCSTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t BCSTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // bcs
    BCSDGP bcsDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setBCSSchedule(bcsDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = bcsDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = bcsDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = bcsDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // bcs
...
///////////////////////////////////////////////////////////////////////////////////////// bcs
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            BCSTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractBCSTX resultConvertBCSTX;
            if(!convert.extractionBCSTransactions(resultConvertBCSTX)){
            ...
            if(!CheckMinGasPrice(resultConvertBCSTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertBCSTX.first, blockGasLimit, pindex->pprev);
            ...
            for(BCSTransaction& qtx : resultConvertBCSTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertBCSTX.first.size(); k ++){
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(500000000 + ((pindex->nHeight - 5000) * 1000)) * COIN){
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // bcs
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // bcs
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // bcs
            globalState->setRootUTXO(oldHashUTXORoot); // bcs
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // bcs
        uint32_t sizeBlockDGP = bcsDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for BCS PoS we need to sync this after every block to ensure txdb is populated for
